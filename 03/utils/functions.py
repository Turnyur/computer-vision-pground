...
def inliers_epipolar_constraint(p1: t_points, p2: t_points, F:t_homography, distance_threshold: float) -> np.array:
    ...
    ones_1 = np.ones((N, 1))
    ones_2 = np.ones((N, 1))
    p1_homogeneous = np.hstack([p1, ones_1])
    p2_homogeneous = np.hstack([p2, ones_2])

    # Step 2 - Compute the epipolar lines in the second image (l2) for each point in the first image
    l2 = np.dot(F, p1_homogeneous.T).T  # l2 is now of shape [N, 3]

    # Step 3 - Normalize the epipolar lines
    l2_normalized = l2 / l2[:, 2:3]  # Normalize by the last element of each line equation

    # Step 4 - Compute the distances from points in p2 to the epipolar lines l2
    distances = np.abs(np.sum(p2_homogeneous * l2_normalized, axis=1))  # Compute distance |l2^T * p2|

    # Calculate distance threshold
    inliers = np.where(distances <= distance_threshold)[0]  # Find indices where distance is within the threshold
...
def triangulate(P1: t_view, P2: t_view, p1: t_points, p2: t_points) -> np.array:
    ...
    # Construct A with the correct rows for both camera matrices
    A = np.array([
        p_x * P1[2] - P1[0],
        p_y * P1[2] - P1[1],
        q_x * P2[2] - P2[0],
    ...
    # The solution is the last row of Vt (smallest singular value)
    X_homogeneous = Vt[-1]  # This is in homogeneous coordinates [X, Y, Z, W]
    
    # Step 3: Convert homogeneous to 3D (inhomogeneous) by dividing by W
    X_inhomogeneous = X_homogeneous[:3] / (X_homogeneous[3] + epsilon)
    
    return X_inhomogeneous    
...
def decompose(E: t_homography) -> Tuple[np.array, np.array, np.array, np.array]:
    ...
    # Step 2 - Compute W and Possible rotations
    W = np.array([[0, -1, 0],
                   [1, 0, 0],
                   [0, 0, 1]])
    R1 = U @ W @ Vt
    ...
    # Step 3 - Possible translations and return R1, R2, t1, t2
    t1 = U[:, 2] / np.linalg.norm(U[:, 2])  # Normalize the third column of U
    t2 = -t1  # The opposite direction
